...
void db_update_coinds(YAAMP_DB *db)
{
	...
	db_query(db, "SELECT id, name, rpchost, rpcport, rpcuser, rpcpasswd, rpcencoding, master_wallet, reward, price, "
		...
		"rpccurl, rpcssl, rpccert, account, multialgos, max_miners, max_shares, usesegwit, dedicatedport "
		"FROM coins WHERE enable AND auto_ready AND algo='%s' AND dedicatedport=%d ORDER BY index_avg", g_stratum_algo, g_tcp_port);
	...
	while((row = mysql_fetch_row(result)) != NULL)
	{
		...
		if(!coind)
		{
			//new coind setting, test if match the fillter roules
		...
		if (row[36] && atoi(row[36]) > 0) coind->dedicatedport = atoi(row[36]);
